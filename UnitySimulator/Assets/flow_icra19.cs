...
public class flow_icra19 : MonoBehaviour
{
    ...
    int[] lane1InitPos; // the initial position of cars on each lane for ICRA19
    int[] lane2InitPos;
    int[] lane3InitPos;
    int[] lane4InitPos;
    ...
    void Start()
    {
        ...
        /// hard-code variables
        //////////////////////////////////
        laneCarNum = new int[] {1, 2, 13, 13 };
        laneFirstCarPos = new float[] { 0f, 0f, 100f, 0f };
        lane1InitPos = new int[] { 40 };
        lane2InitPos = new int[] { -90, 1, };
        lane3InitPos = new int[] { 550, 500, 450, 400, 350, 300, 280, 255, 230, 200, 180, 130, 80 }; 
        lane4InitPos = new int[] { 500, 450, 400, 350, 330, 305, 280, 250, 210, 190, 170, 140, 110 }; 
        Debug.Log("asdf3");
        //////////////////////////////////
        ...
        for (int i = 0; i < laneCarNum.Length; i++)
        {
            if (laneCarNum[i] > 0)
            {
                ...
                {
                    case 0:
                        generateTraffic(i, ref lane0Pos, ref lane0Vel, ref lane0VelGoal, ref lane0Len, ref lane0Obj, lane1InitPos);
                        break;
                    case 1:
                        generateTraffic(i, ref lane1Pos, ref lane1Vel, ref lane1VelGoal, ref lane1Len, ref lane1Obj, lane2InitPos);
                        break;
                    case 2:
                        generateTraffic(i, ref lane2Pos, ref lane2Vel, ref lane2VelGoal, ref lane2Len, ref lane2Obj, lane3InitPos);
                        break;
                    case 3:
                        generateTraffic(i, ref lane3Pos, ref lane3Vel, ref lane3VelGoal, ref lane3Len, ref lane3Obj, lane4InitPos);
                        break;
                    default:
                        break;
                }
    ...
    private void generateTraffic(int laneIdx, ref List<Vector3> lanePos, ref List<float> laneVel, ref List<float> laneVelGoal, ref List<float> laneLength, ref List<GameObject> laneObj, int[] laneInitPos)
    {
        ...
        for (int i = 0; i < laneCarNum[laneIdx]; i++)
        {
            ...
            {
                carObj.transform.position = new Vector3(laneCtrPos[laneIdx], 0f, laneInitPos[i]);
