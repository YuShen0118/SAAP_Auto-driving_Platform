...
class GameState:
    def __init__(self):
        ...
        #self.create_agt((400, 200))
        #self.create_agt((100, 300))
        
        self.traj_all = []
        for i in range(0, agt_num):
            self.traj_all.append(self.LoadTraj(i))
        
        self.agt_all = []
        for i in range(0, agt_num):
            self.agt_all.append(self.CreateAgt(self.traj_all[i][0]))
        
                
    def frame_step(self, action):
        
        if(self.num_steps < len(self.traj_all[0])):
            for i in range(0, agt_num):
                self.agt_all[i].position = self.traj_all[i][self.num_steps]
    ...
    def LoadTraj(self, agt_id):
        traj = [(1,1), (2,2)]
        '''
        file_name = 'C:/traj_data/traj' + str(agt_id)
        with open(file_name, "r") as filestream:
            for line in filestream:
                entry = line.split(",")
                traj.append((float(entry[0]) * agt_scale + width/2, float(entry[1])* agt_scale + height/2))
        '''
    ...
    def CreateAgt(self, pos):
        agt_mass = 1
        size_shape_ratio = agt_scale / 25
        agt_size = (30*size_shape_ratio, 50*size_shape_ratio)
        agt_inertia = pymunk.moment_for_circle(agt_mass, 0, 14, (0, 0))
        agt_body = pymunk.Body(agt_mass, agt_inertia)
        agt_body.position = pos
        agt_shape = pymunk.Circle(agt_body, agt_scale)
        agt_shape.color = THECOLORS["red"]
        self.space.add(agt_body, agt_shape)
        return agt_body
    ...
    def move_cat(self):
        ...
        direction = Vec2d(1, 0).rotated(self.cat_body.angle)
        self.cat_body.velocity = speed * direction * 10
